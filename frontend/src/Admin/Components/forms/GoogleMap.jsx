...
const GoogleMap = ({ editHandler, componentType,mapValues }) => {
  ...
  const [google_map_url, setGoogle_map_url]= useState(mapValues.google_map_url)
  ...
  const saveGoogleMapUrl =async() => {
    ...
    try {
      ...
      if (mapValues.id) {
        ...
        response = await axiosServiceApi.post(`/footer/createGoogleMapURL/`, data);
      }
          ...
          <div className="row py-0 pb-md-5">
            ...
            <div className="row">
              <div className="text-center">
                ...
                <button onClick={saveGoogleMapUrl} className="btn btn-primary">Save</button>
              </div>
            </div>
          </div>
        </div>
